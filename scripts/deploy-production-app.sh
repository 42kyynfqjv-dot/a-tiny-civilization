...
while (($#)); do
  case "$1" in
    ...
    --confirm-public-deployment)
      ...
      ;;
    *)
      echo "usage: $0 [--env-file /absolute/path/to/production.env] --genesis-directory /absolute/path --evidence-directory /absolute/path [--admission-file /absolute/path] [--observatory-admission-file /absolute/path] [--runtime-root /absolute/path] --confirm-public-deployment" >&2
      exit 2
      ;;
  esac
done
...
if [[ "$observatory_admission" != /* || ! -f "$observatory_admission" || -L "$observatory_admission" ]]; then
  echo "deployment requires an absolute, regular observatory-admission file" >&2
  exit 2
fi
