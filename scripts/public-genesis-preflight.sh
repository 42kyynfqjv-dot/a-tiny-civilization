...
usage() {
  echo "usage: $0 --genesis-directory PATH --evidence-directory PATH [--env-file PATH] [--admission-file PATH] [--runtime-root PATH]" >&2
  exit 2
}
...
for path in "$environment_file" "$genesis_directory" "$evidence_directory" "$admission_file" "$runtime_root"; do
  if [[ "$path" != /* ]]; then
