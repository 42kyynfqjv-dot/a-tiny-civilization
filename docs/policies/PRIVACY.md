...
If remote cognition is enabled, the disclosed model provider processes private simulated-state
requests and bounded recalled-memory context for civilization inhabitants. Those requests do not
contain observer accounts, supporter aliases, payment records, or visitor telemetry. Remote
cognition remains disabled unless the operator separately approves that export and identifies the
processor in this notice before public activation.
